...
/**
 * Servlet implementation class ExibirListaProdutosBuscados
 */
@WebServlet("/ExibirListaProdutosBuscados")
public class ExibirListaProdutosBuscados extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			for (Produto p : dao.listar()) {
				if(p.getNome().toLowerCase().contains(busca.toLowerCase()) || (p.getCodigo()+"").equalsIgnoreCase(busca) || p.getNome().equalsIgnoreCase(busca) || busca.equals("")) {
					produtos.add(p);
				}
			}
